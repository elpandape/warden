...
it('resolves custom models from the configuration', function (): void {
    config()->set('bouncer.models.role', CustomRole::class);
...
it('fails fast when the override is not an eloquent model class', function (): void {
    config()->set('bouncer.models.role', 'Not\A\Real\Class');
    app()->forgetInstance(Context::class);

    Context::resolve()->modelClass('role');
})->throws(InvalidArgumentException::class, 'Configured bouncer model [role]');
...
it('prefers the configured user model over the auth guard', function (): void {
    config()->set('bouncer.models.user', ElPandaPe\Bouncer\Tests\Fixtures\Account::class);
    app()->forgetInstance(Context::class);

    expect(Context::resolve()->modelClass('user'))->toBe(ElPandaPe\Bouncer\Tests\Fixtures\Account::class);
});
...
})->throws(InvalidArgumentException::class, 'bouncer.models.user');
...
it('re-registers the morph alias when a model is swapped at runtime', function (): void {
    ...
    expect(Illuminate\Database\Eloquent\Relations\Relation::getMorphedModel('bouncer.role'))->toBe(CustomRole::class)
        ->and((new CustomRole)->getMorphClass())->toBe('bouncer.role');
});

afterEach(function (): void {
    // setModelClass may repoint the static morph map; restore the package default.
    Illuminate\Database\Eloquent\Relations\Relation::morphMap(['bouncer.role' => Role::class]);
});
